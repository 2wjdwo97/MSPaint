...
//	private Color color_fill = Color.white;
	...
	class upListener implements ActionListener {
		public void actionPerformed(ActionEvent e) {
			...
			if (focus != -1 && focus != theModel.size() - 2) {
				...
				theModel.add(new SlctArea(theModel.get(focus).getXpos_1(), theModel.get(focus).getXpos_2(),
						theModel.get(focus).getYpos_1(), theModel.get(focus).getYpos_2(),
						theModel.get(focus).getState(), theModel.get(focus)));
				theView.repaint();
			}
		}
	}
	...
	class createMenuListener implements ActionListener {
		public void actionPerformed(ActionEvent e) {
			if (theModel.get(focus).getSelectedPoint() == theModel.get(focus).getXpolyPoints().size() - 1) {
				theModel.get(focus).getXpolyPoints().add(theModel.get(focus).getSelectedPoint() + 1,
						(theModel.get(focus).getXpolyPoints().get(theModel.get(focus).getSelectedPoint())
								+ theModel.get(focus).getXpolyPoints().get(0)) / 2);
				theModel.get(focus).getYpolyPoints().add(theModel.get(focus).getSelectedPoint() + 1,
						(theModel.get(focus).getYpolyPoints().get(theModel.get(focus).getSelectedPoint())
								+ theModel.get(focus).getYpolyPoints().get(0)) / 2);

			} else {
				theModel.get(focus).getXpolyPoints().add(theModel.get(focus).getSelectedPoint() + 1,
						(theModel.get(focus).getXpolyPoints().get(theModel.get(focus).getSelectedPoint())
								+ theModel.get(focus).getXpolyPoints().get(theModel.get(focus).getSelectedPoint() + 1))
								/ 2);
				theModel.get(focus).getYpolyPoints().add(theModel.get(focus).getSelectedPoint() + 1,
						(theModel.get(focus).getYpolyPoints().get(theModel.get(focus).getSelectedPoint())
								+ theModel.get(focus).getYpolyPoints().get(theModel.get(focus).getSelectedPoint() + 1))
								/ 2);
			}
	...
	class deleteMenuListener implements ActionListener {
		public void actionPerformed(ActionEvent e) {
			for (int i = theModel.get(focus).getSelectedPoint(); i < theModel.get(focus).getXpolyPoints().size()
					- 1; i++) {
	...
	class colorFillListener implements ActionListener {
		...
		public void actionPerformed(ActionEvent e) {
			...
			ShapeFactory.setColor_fill(selectedColor);
	...
	class colorLineListener implements ActionListener {
		...
		public void actionPerformed(ActionEvent e) {
			...
			ShapeFactory.setColor_line(selectedColor);
	...
	class rectListener implements ActionListener {
		public void actionPerformed(ActionEvent e) {
			drawingTool = new DrawRectangle();
			if (focus != -1) {
				theView.setEditPointButton(false);
				focus = -1;
				theModel.remove(theModel.size() - 1);// selectArea remove
				theView.repaint();
			}
		}
	}
	...
	class editPointListener implements ActionListener {
		public void actionPerformed(ActionEvent e) {
			...
			theModel.add(new SlctArea(focusModel.getXpos_1(), focusModel.getXpos_2(), focusModel.getYpos_1(),
					focusModel.getYpos_2(), focusModel.getState(), focusModel));
			theView.repaint();
		}
	}
		...
		public void mouseDragged(MouseEvent e) {
			if (e.getButton() == MouseEvent.BUTTON3) {
				...
				if (focus != -1) { // 선택된 도형이 있을 때
					...
					if (drawMode >= 0) { // 그리기면
						if (drawMode == RECTANGLE || drawMode == ELLIPSE || drawMode == TRIANGLE
								|| drawMode == DIAMOND) {
	...
	class clickListener implements MouseListener {
		...
		public void mousePressed(MouseEvent e) {
			...
			if (e.getButton() == MouseEvent.BUTTON3 && focus != -1) {
				...
				rotate_x = selectedValue.rotateMousePointer_x(e.getX(), e.getY());
				rotate_y = selectedValue.rotateMousePointer_y(e.getX(), e.getY());
				if (theModel.get(focus).getPointEdit()
						&& theModel.get(theModel.size() - 1).isInside(rotate_x, rotate_y))
				...
				drawingTool = drawingTool.pressed(theModel, focus, e.getX(), e.getY());
			...
			if (e.getButton() == MouseEvent.BUTTON3) {
				...
				if (drawMode != POLYGON) {
					if (focus != -1) {
						...
						theModel.add(new SlctArea(focusModel.getXpos_1(), focusModel.getXpos_2(),
								focusModel.getYpos_1(), focusModel.getYpos_2(), focusModel.getState(), focusModel));
					}
					drawMode = SELECT;
					theView.repaint();
				}
